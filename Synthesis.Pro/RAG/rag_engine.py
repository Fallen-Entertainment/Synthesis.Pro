...
class SynthesisRAG:
    ...
    def _setup_local_embeddings(self):
        ...
        try:
            result = subprocess.run(
                ["sqlite-rag", "download-model", model_name, model_file],
                capture_output=True,
                text=True,
                check=False
            )
    ...
    def add_text(self, text: str, private: bool = True) -> bool:
        ...
        ⚠️ SAFETY: Defaults to PRIVATE to prevent accidental data leaks!
        ...
        if not private:
            print(f"⚠️  WARNING: Adding to PUBLIC database!")
            print(f"   Make sure this content is safe to share!")

        try:
            result = subprocess.run(
                ["sqlite-rag", "--database", database, "add-text", text],
                capture_output=True,
                text=True,
                check=True
            )
            ...
            print(f"❌ Error adding text to {db_type} database: {e.stderr}")
            return False
    ...
    def add_documents(self, paths: List[str], recursive: bool = True, private: bool = True) -> bool:
        ...
        # Safety check: Warn if adding to public
        if not private:
            print(f"⚠️  WARNING: Adding documents to PUBLIC database!")
            print(f"   Files: {paths}")
        ...
        try:
            for path in paths:
                cmd = ["sqlite-rag", "--database", database, "add", path]
                if recursive:
    ...
    def add_public_solution(self, problem: str, solution: str, tags: str = "") -> bool:
        ...
        ⚠️ IMPORTANT: Sanitize before adding! Remove:
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("✅ Synthesis.Pro Dual Database Demo Complete!")
    print("=" * 60)
    print("\n💡 Key Benefits:")
